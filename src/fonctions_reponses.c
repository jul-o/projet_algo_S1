...
// Pour chaque case du dallage, on regarde tous les plus grand rectangles qu'on
// puisse faire en utilisant la case de dallage actuel comme coin supérieur
// gauche Dès qu'on a trouver la largeur max, on se rapelle de ce point, il
// pourra servir comme coint inférieur droit A la fin on compare la surface
// qu'on obtient avec tous les coins inférieurs droits, et on garde celui qui
// permet la plus grande surface
    ...
    for (int x0 = 0; x0 < 1; x0++) {
        for (int y0 = 0; y0 < 1; y0++) {
            ...
            while (y1 < tiles->lines && tiles->values[x1][y1] != 1) {
                while (x1 < tiles->columns && tiles->values[x1][y1] != 1 && x1 < max_x)
                    x1++;
                if (tiles->values[x1][y1] == 1)
                    x1--;  // If the last one was a 1, don't count it
                x_found[nb_found] = x1;
                y_found[nb_found] = y1;
                nb_found++;
